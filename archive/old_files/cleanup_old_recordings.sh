...
count_files() {
    find "$AUDIO_DIR" -type f \( -name "*.mp3" -o -name "*.flac" -o -name "*.json" \) | wc -l
}
...
# Find and remove audio files (.mp3 and .flac) older than 7 days
REMOVED_AUDIO=$(find "$AUDIO_DIR" -type f \( -name "*.mp3" -o -name "*.flac" \) -mtime +$DAYS_OLD -print)
if [ -n "$REMOVED_AUDIO" ]; then
    echo "$REMOVED_AUDIO" | while read -r file; do
        log_message "Removing audio file: $file"
        rm -f "$file"
    done
    AUDIO_COUNT=$(echo "$REMOVED_AUDIO" | wc -l)
else
    AUDIO_COUNT=0
fi
...
# Log summary
log_message "Cleanup completed: Removed $AUDIO_COUNT audio files and $JSON_COUNT metadata files"
